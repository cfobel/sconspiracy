...
from yams.yproject import ConstructibleYamsProject, LibName
from yams.yutils   import cached_property, memoize, run_once


class CppUnitError(yams.YamsProjectError):
    pass

CPPUNIT_PLUGIN_PATH = os.path.dirname(__file__)


class CppUnitProject(ConstructibleYamsProject):
    cppunit_test_dir    = 'tu'
    ...
    def __init__(self, prj, config=None, **kwargs):
        if not isinstance(prj,ConstructibleYamsProject):
            msg = ('CppUnitProject take a '
                   'ConstructibleYamsProject as first argument')
...
#    @cached_property
    ...
    def configure_env(self):
        yams.ylibext.configure(self, ['cppunit'])
    ...
    def result (self, deps_results):
        ...
        if self.get_lower(self.test_run_var_name) == 'yes':
            ...
            env_var = env_var[yams.yenv.system()]
