...
class DepedenciesError(yams.YamsProjectError):
    pass

DEPENDENCIES_PLUGIN_PATH = os.path.dirname(__file__)

class DepedenciesProject(ConstructibleYamsProject):
    var_name = 'DEPS'
    ...
    def install (self, opts = ['rc', 'deps'] ):
        ...
        yams.print_msg(self.base_name + ':' + msg)
        yams.print_msg(self.base_name + ' libext:' + ' '.join(uses))
        return result
