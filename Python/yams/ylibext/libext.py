...
class LibExt(object):
    ...
    arch           = yams.yenv.options.get_option('ARCH')

    depends_on     = []

    basepath       = yams.yenv.dirs.libext
    ...
    def __init__(self, name, debug, infosource=None):
        func = getattr(self, yams.yenv.system(), self.not_managed)
        ...
        self.version = yams.yutils.Version(infosource.version)
    ...
    def not_managed(self):
        msg = '<{0}> system is not managed.'.format(yams.yenv.system())
        from yams import LibExtException
