...
import yams.ylibext       as ylibext
import yams.yutils        as yutils

from yams import ConfigVariantError,\
                 LibBadVersion,\
                 LibMissingVersion,\
                 NotUsed,\
                 YamsProjectError\

from yams.yenv.configs import get_config, allowedvalues
from yams.yenv         import constants
from yams.yutils       import cached_property, memoize, run_once


all = ['LibName', 'YamsProject', 'ConstructibleYamsProject']
...
@yams.no_undef_attr_read
@yams.no_undef_attr_write
class YamsProject(object):
    """Represents a yams project, read the options and provide some usefull
    ...
    def __init__(self, options_file, 
        ...
        if not os.path.isfile(options_file):
            raise YamsProjectError( self, '{prj._opts_file} is not a file')
        ...
        if not config:
            global_config = yenv.options.get_option( "CONFIG" )
    ...
    def get(self, opt):
        ...
        defined value). see yenv/options"""
        kwargs = {}
    ...
    def full_postfix(self):
        ...
        arch        = yenv.options.get_option('ARCH')
    ...
    @cached_property
    def version (self):
        return yutils.Version(self.get('VERSION'))
    ...
    def type (self):
        ...
        if type not in allowedvalues.TYPE:
            raise YamsProjectError( self, 'TYPE must be defined.')
        return type
    ...
    @cached_property
    def is_console (self):
        return yutils.is_true(self.get_lower('CONSOLE'))
    ...
    def _get_libnames(self, value):
        ...
        miss = [el for el in miss if not ylibext.is_available(el)]

        if miss :
            raise YamsProjectError(self,
                    ('Missing <' + ', '.join(miss) + '>. '
...
#            raise LibMissingVersion(missing_versions, self)
    ...
    @cached_property
    def build_dir(self):
        return pathjoin(yenv.dirs.build, self.full_name)
    ...
    def install_rc_path(self):
        if self.type in ALLEXEC + ALLLIB + BINLIBEXT:
            install_dir = yenv.dirs.install_share
        elif self.type in ALLBUNDLE:
            install_dir = yenv.dirs.install_bundle
        else:
            raise YamsProjectError ( self,
    ...
    def install_path(self):
        if self.type in ALLEXEC:
            install_dir = yenv.dirs.install_bin
        elif self.type in ALLLIB + BINLIBEXT:
            install_dir = yenv.dirs.install_lib
        elif self.type in ALLBUNDLE:
            install_dir = pathjoin(yenv.dirs.install_bundle,
                    self.versioned_name)
        else:
            raise YamsProjectError ( self,
    ...
    def install_pkg_path(self):
        if self.type in ALLEXEC + ALLLIB + BINLIBEXT:
            install_dir = yenv.dirs.install_binpkg
        else:
            raise YamsProjectError ( self,
...
class InstallableYamsProject(YamsProject):

    env = None

    def __init__(self, options_file, env, **kwargs):
        self.env = env.Clone()
        super(InstallableYamsProject,self).__init__(
    ...
    def install_files (self, src_path, dst_path, ext):
        ...
        if os.path.isdir(src_path):
            sources_files, install_files = yutils.DeepGlob(
                                            extensions  = ext,
                                            src_dir     = src_path,
...
class ConstructibleYamsProject(InstallableYamsProject):
    ...
    def exports(self, export=False):
        ...
        if self.get('USEVISIBILITY') == "yams":
...
#        import pipes
        ...
        if not prj.is_exec:
            if self.get('USEVISIBILITY') in ["no", "yams"]:
                deps = prj.source_libs_deps + prj.source_bundles_deps

                # XXX Yams visibility transition ###########################
    ...
    def configure_env(self):
        ...
        ylibext.configure(prj, rec_uses, opts=['nolink'])
        ylibext.configure(prj, uses, opts=uses_opts)
    ...
    def install (self, opts = ['deps','rc']):
        ...
        """
        if not yutils.is_true(self.get('BUILD')):
            return []
        ...
        result += super(ConstructibleYamsProject,self).install(opts)
    ...
    def install_pkg(self):
        env = self.env
        res = super(ConstructibleYamsProject,self).install_pkg()
