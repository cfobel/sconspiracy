...
import yams.yplugins as yplug

from yams.yenv             import constants
from yams.yenv.options     import get_option
from yams.yproject.project import ConstructibleYamsProject, \
                                  InstallableYamsProject
from yams.yutils           import memoize, remove_vcs_dirs
...
class YamsProjectsDB(object):
    ...
    def __init__(self, directory_list=[], env={}, 
        ...
        if not directory_list:
            directory_list = yams.yenv.dirs.code
    ...
    def _register_prj(self, prj):
        if prj.register_name in self._prj_map:
            ...
            msg = """An existing project is already called <{0.register_name}>.
                  ...
                  """.format(prj, prev_prj)
            yams.print_warning( 'Project {0.full_name}'.format(prj), msg)
    ...
    def _make_prj_from_file(self, file, args = {},
            factory = ConstructibleYamsProject):
        kwargs = {}
    ...
    def _register_prj_from_file(self, file):
        ...
        target = yams.yenv.TARGETS.get(name)
    ...
    def target_lookup(self, name, **kw):
        ...
        try:
            ...
            if target.name and db.has_key(target.name):
                ...
                res = []
                for p in to_build:
                    builddeps = yams.yutils.is_true(p.get('BUILDDEPS'))
                    buildpkg  = yams.yutils.is_true(p.get('BUILDPKG'))
                    opts = ['rc']
                    opts += ['deps'] if builddeps else []
                    opts += ['pkg'] if buildpkg else []
                    res += p.install(opts = opts) 

                for libext in yams.ylibext.configure.configured.values():
                    if hasattr(libext, '__src__'):
                        ...
                        if os.path.exists(buildoptions):
                            libextprj = self._make_prj_from_file(buildoptions,
                                    factory=InstallableYamsProject)
                ...
                yams.print_warning('Unknown target', msg)

        except yams.YamsException, e:
            yams.manage_exception(e)
            exit(1)
